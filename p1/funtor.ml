...
module Conjunto (MiTipo : Conjunto.TipoOrdenado) : (Conjunto.CONJUNTO with type elemento = MiTipo.t )=
struct
  type elemento = MiTipo.t
  
  type conjunto = elemento list

  let cmp = MiTipo.comparar
